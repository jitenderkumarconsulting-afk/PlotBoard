...
[SampleApp](http://plotboard.com) Service-Edit-Game

## Description

The Service-Edit-Game is a micro-service API that facilitates the updation of exiting games for the SampleApp project. It is an integral part of the SampleApp platform, which aims to provide an immersive gaming experience to its users. The micro-service is built using the NestJS framework, a progressive Node.js framework that offers a modular architecture and promotes code reusability.

With the Service-Edit-Game micro-service, users can easily update existing games and set up their unique gaming experiences. It leverages MongoDB, a popular NoSQL database, to efficiently store and retrieve game-related data, ensuring high performance and scalability.

## Tech Stack

The Service-Edit-Game micro-service utilizes the following technologies:
...
To install and run the SampleApp Service-Edit-Game micro-service locally, follow these steps:
...
5. Clone the repository:
   ...
   ```bash
   $ git clone https://github.com/ned-plotboard/service-edit-game.git
   ```

6. Install the dependencies:

   ```bash
   $ cd service-edit-game
   $ npm install
   ```
...
To start the Service-Edit-Game micro-service, run the following command:
...
By default, the micro-service will start on port 7005. You can access the API endpoints using the base URL: **http://localhost:7005**.

## Test

To run the tests for the Service-Edit-Game micro-service, use the following command:
...
The Service-Edit-Game micro-service provides the following endpoints:

- **PATCH /games/:id:** Update an existing game by ID.

Refer to the [Swagger UI](http://localhost:7005/swagger-ui) documentation for detailed information on the request and response payloads of each endpoint. The Swagger UI provides an interactive interface to explore and test the API endpoints.

## Authentication

The Service-Edit-Game micro-service uses JWT (JSON Web Tokens) for authentication. When a user signs in, an access token is generated and returned in the response. This token should be included in the **`Authorization`** header of subsequent requests as a bearer token.
