...
[SampleApp](http://plotboard.com) Service-My-Games

## Description

The Service-My-Games is a micro-service API that facilitates the fetching of exiting games for the SampleApp project. It is an integral part of the SampleAppSampleApp platform, which aims to provide an immersive gaming experience to its users. The micro-service is built using the NestJS framework, a progressive Node.js framework that offers a modular architecture and promotes code reusability.

With the Service-My-Games micro-service, users can easily fetch existing games and set up their unique gaming experiences. It leverages MongoDB, a popular NoSQL database, to efficiently store and retrieve game-related data, ensuring high performance and scalability.

## Tech Stack

The Service-My-Games micro-service utilizes the following technologies:
...
To install and run the SampleApp Service-My-Games micro-service locally, follow these steps:
...
5. Clone the repository:
   ...
   ```bash
   $ git clone https://github.com/ned-plotboard/service-my-games.git
   ```
...
To start the Service-My-Games micro-service, run the following command:
...
By default, the micro-service will start on port 7006. You can access the API endpoints using the base URL: **http://localhost:7006**.

## Test

To run the tests for the Service-My-Games micro-service, use the following command:
...
The Service-My-Games micro-service provides the following endpoints:

- **GET /games:** Fetch all existing games of the authenticated user.
- **GET /games/:id:** Fetch an existing game by ID.

Refer to the [Swagger UI](http://localhost:7006/swagger-ui) documentation for detailed information on the request and response payloads of each endpoint. The Swagger UI provides an interactive interface to explore and test the API endpoints.

## Authentication

The Service-My-Games micro-service uses JWT (JSON Web Tokens) for authentication. When a user signs in, an access token is generated and returned in the response. This token should be included in the **`Authorization`** header of subsequent requests as a bearer token.
