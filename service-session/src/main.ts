...
function setupSwagger(
  ...
  const options = new DocumentBuilder()
    .setTitle(appName)
    .setDescription(
      `This API provides endpoints for managing ${appName}, allowing users to manage user session. It follows RESTful principles and uses JSON as the data exchange format.`,
    )
  ...
  const swaggerUrl = `http://localhost:${process.env.PORT || 7003}/swagger-ui`;
