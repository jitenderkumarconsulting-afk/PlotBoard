...
[SampleApp](http://plotboard.com) Service-Signup

## Description

The Service-Signup is a micro-service API that handles user signup and authentication for the SampleApp project. It is built using the NestJS framework and utilizes MongoDB as the database. The micro-service is containerized using Docker and can be deployed using Docker Compose.

The purpose of this micro-service is to provide a reliable and secure signup process for new users of the SampleApp platform. It handles user registration, validation of user data, and authentication using industry-standard practices.

## Tech Stack

The Service-Signup micro-service utilizes the following technologies:
...
- **SendGrid:** A cloud-based email service that provides reliable delivery, scalability, and flexibility for sending transactional and marketing emails. The micro-service utilizes the SendGrid service to send email notifications for user signup and account verification.

## Installation

To install and run the SampleApp Service-Signup micro-service locally, follow these steps:
...
5. Clone the repository:
   ...
   ```bash
   $ git clone https://github.com/ned-plotboard/service-signup.git
   ```
...
To start the Service-Signup micro-service, run the following command:
...
By default, the micro-service will start on port 7001. You can access the API endpoints using the base URL: **http://localhost:7001**.

## Test

To run the tests for the Service-Signup micro-service, use the following command:
...
The Service-Signup micro-service provides the following endpoints:

- **POST /auth/sign-up:** Register a new user and generate an access token.

Refer to the [Swagger UI](http://localhost:7001/swagger-ui) documentation for detailed information on the request and response payloads of each endpoint. The Swagger UI provides an interactive interface to explore and test the API endpoints.

## Authentication

The Service-Signup micro-service uses JWT (JSON Web Tokens) for authentication. When a user signs up, an access token is generated and returned in the response. This token should be included in the **`Authorization`** header of subsequent requests as a bearer token.
