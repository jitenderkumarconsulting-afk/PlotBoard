...
[SampleApp](http://plotboard.com) Service-User-Info

## Description

The Service-User-Info micro-service is a vital component of the SampleApp project, built using NestJS and MongoDB. It efficiently retrieves user information for authenticated users, ensuring security and reliability. Containerized with Docker and deployable via Docker Compose, it offers scalability.

Committed to industry standards, the micro-service optimizes user info retrieval, enhancing the SampleApp platform's performance. It exemplifies the platform's dedication to advanced technology and meticulous development, ultimately enriching user satisfaction.

## Tech Stack

The Service-User-Info micro-service utilizes the following technologies:
...
To install and run the SampleApp Service-User-Info micro-service locally, follow these steps:
...
5. Clone the repository:
   ...
   ```bash
   $ git clone https://github.com/ned-plotboard/service-user-info.git
   ```

6. Install the dependencies:

   ```bash
   $ cd service-user-info
   $ npm install
   ```
...
To start the Service-User-Info micro-service, run the following command:
...
By default, the micro-service will start on port 7008. You can access the API endpoints using the base URL: **http://localhost:7008**.

## Test

To run the tests for the Service-User-Info micro-service, use the following command:
...
The Service-User-Info micro-service provides the following endpoints:

- **GET /user/info:** Fetch user info of the authenticated user.

Refer to the [Swagger UI](http://localhost:7008/swagger-ui) documentation for detailed information on the request and response payloads of each endpoint. The Swagger UI provides an interactive interface to explore and test the API endpoints.

## Authentication

The Service-User-Info micro-service uses JWT (JSON Web Tokens) for authentication. When a user signs in, an access token is generated and returned in the response. This token should be included in the **`Authorization`** header of subsequent requests as a bearer token.
