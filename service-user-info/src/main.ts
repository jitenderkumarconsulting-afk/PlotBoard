...
function setupSwagger(
  ...
  const options = new DocumentBuilder()
    .setTitle(appName)
    .setDescription(
      `This API provides endpoints for managing ${appName}, allowing authenticated users to fetch user info. It follows RESTful principles and uses JSON as the data exchange format.`,
    )
  ...
  const swaggerUrl = `http://localhost:${process.env.PORT || 7008}/swagger-ui`;
