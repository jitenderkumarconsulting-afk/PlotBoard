...
[SampleApp](http://plotboard.com) Service-Signin

## Description

The Service-Signin is a micro-service API that handles user authentication for the SampleApp project. It is built using the NestJS framework and utilizes MongoDB as the database. The micro-service is containerized using Docker and can be deployed using Docker Compose.

The purpose of this micro-service is to provide a reliable and secure signin process for new users of the SampleApp platform. It handles user authentication using industry-standard practices.

## Tech Stack

The Service-Signin micro-service utilizes the following technologies:
...
To install and run the SampleApp Service-Signin micro-service locally, follow these steps:
...
5. Clone the repository:
   ...
   ```bash
   $ git clone https://github.com/ned-plotboard/service-signin.git
   ```
...
To start the Service-Signin micro-service, run the following command:
...
By default, the micro-service will start on port 7002. You can access the API endpoints using the base URL: **http://localhost:7002**.

## Test

To run the tests for the Service-Signin micro-service, use the following command:
...
The Service-Signin micro-service provides the following endpoints:

- **POST /auth/sign-in:** Authenticate a user and generate an access token.

Refer to the [Swagger UI](http://localhost:3000/swagger-ui) documentation for detailed information on the request and response payloads of each endpoint. The Swagger UI provides an interactive interface to explore and test the API endpoints.

## Authentication

The Service-Signin micro-service uses JWT (JSON Web Tokens) for authentication. When a user signs in, an access token is generated and returned in the response. This token should be included in the **`Authorization`** header of subsequent requests as a bearer token.
