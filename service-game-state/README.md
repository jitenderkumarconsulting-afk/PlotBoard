...
[SampleApp](http://plotboard.com) Service-Game-State

## Description

The Service-Game-State is a micro-service API that handles game state management for the SampleApp project. It is built using the NestJS framework and utilizes MongoDB as the database. The micro-service is containerized using Docker and can be deployed using Docker Compose.

The purpose of this micro-service is to provide a reliable and secure state management process for games of the SampleApp platform. It handles game state management using industry-standard practices.

## Tech Stack

The Service-Game-State micro-service utilizes the following technologies:
...
To install and run the SampleApp Service-Game-State micro-service locally, follow these steps:
...
5. Clone the repository:
   ...
   ```bash
   $ git clone https://github.com/ned-plotboard/service-game-state.git
   ```

6. Install the dependencies:

   ```bash
   $ cd service-game-state
   $ npm install
   ```
...
To start the Service-Game-State micro-service, run the following command:
...
By default, the micro-service will start on port 7010. You can access the API endpoints using the base URL: **http://localhost:7010**.

## Test

To run the tests for the Service-Game-State micro-service, use the following command:
...
The Service-Game-State micro-service provides the following endpoints:

- **POST /play/state/:urlCode:** Create a game state if not exists for play game URL code.
- **GET /play/state/:urlCode:** Fetch an existing game state by play game URL code.

Refer to the [Swagger UI](http://localhost:7010/swagger-ui) documentation for detailed information on the request and response payloads of each endpoint. The Swagger UI provides an interactive interface to explore and test the API endpoints.

## Authentication

The Service-Game-State micro-service uses JWT (JSON Web Tokens) for authentication. When a user signs in, an access token is generated and returned in the response. This token should be included in the **`Authorization`** header of subsequent requests as a bearer token.
