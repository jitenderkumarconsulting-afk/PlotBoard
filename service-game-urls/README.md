...
[SampleApp](http://plotboard.com) Service-Game-Urls

## Description

The Service-Game-Urls micro-service is a crucial API within the SampleApp project, focusing on game URL management. It's a fundamental element of the SampleApp platform, designed to deliver immersive gaming encounters to users. Developed using the NestJS framework, this micro-service follows best practices, featuring a modular architecture and maximizing code reusability.

Through the Service-Game-Urls micro-service, users effortlessly oversee game URLs, crafting personalized gaming journeys. Its integration with MongoDB, a renowned NoSQL database, guarantees efficient storage and retrieval of game URL data, prioritizing performance and scalability for an enhanced user experience.

## Tech Stack

The Service-Game-Urls micro-service utilizes the following technologies:
...
To install and run the SampleApp Service-Game-Urls micro-service locally, follow these steps:
...
5. Clone the repository:
   ...
   ```bash
   $ git clone https://github.com/ned-plotboard/service-game-urls.git
   ```

6. Install the dependencies:

   ```bash
   $ cd service-game-urls
   $ npm install
   ```
...
To start the Service-Game-Urls micro-service, run the following command:
...
By default, the micro-service will start on port 7009. You can access the API endpoints using the base URL: **http://localhost:7009**.

## Test

To run the tests for the Service-Game-Urls micro-service, use the following command:
...
The Service-Game-Urls micro-service provides the following endpoints:

- **POST /games/:gameId/urls:** Create a new game URL.
- **GET /games/:gameId/urls:** Fetch all game URLs for a specific game.
- **DELETE /games/:gameId/urls/:gameUrlId:** Delete an existing game URL by ID.
- **POST /games/actions/url-code/:urlCode:** Perform game URL code action.

Refer to the [Swagger UI](http://localhost:7009/swagger-ui) documentation for detailed information on the request and response payloads of each endpoint. The Swagger UI provides an interactive interface to explore and test the API endpoints.

## Authentication

The Service-Game-Urls micro-service uses JWT (JSON Web Tokens) for authentication. When a user signs in, an access token is generated and returned in the response. This token should be included in the **`Authorization`** header of subsequent requests as a bearer token.
