...
function setupSwagger(
  ...
  const options = new DocumentBuilder()
    .setTitle(appName)
    .setDescription(
      `This API provides endpoints for managing ${appName}, allowing users to manage game loop events. It follows RESTful principles and uses JSON as the data exchange format.`,
    )
  ...
  const swaggerUrl = `http://localhost:${process.env.PORT || 7011}/swagger-ui`;
...
async function startServer(
  ...
  logger.log(`Socket is running at ${socketUrl}`);
}
