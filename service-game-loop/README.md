...
[SampleApp](http://plotboard.com) Service-Game-Loop

## Description

The Service-Game-Loop is a micro-service API that handles game loop events management for the SampleApp project. It is built using the NestJS framework and utilizes MongoDB as the database. The micro-service is containerized using Docker and can be deployed using Docker Compose.

The purpose of this micro-service is to provide a reliable and secure loop events management process for games of the SampleApp platform. It handles game loop events management using industry-standard practices.

## Tech Stack

The Service-Game-Loop micro-service utilizes the following technologies:
...
- **Redis:** An in-memory data store that serves multiple purposes within the micro-service. It acts as a caching mechanism, accelerating read operations, and also facilitates publish-subscribe messaging for real-time communication.

## Installation

To install and run the SampleApp Service-Game-Loop micro-service locally, follow these steps:
...
5. Clone the repository:
   ...
   ```bash
   $ git clone https://github.com/ned-plotboard/service-game-loop.git
   ```

6. Install the dependencies:

   ```bash
   $ cd service-game-loop
   $ npm install
   ```
...
To start the Service-Game-Loop micro-service, run the following command:
...
By default, the micro-service will start on port 7011. You can access the API endpoints using the base URL: **http://localhost:7011**.

## Test

To run the tests for the Service-Game-Loop micro-service, use the following command:
...
The Service-Game-Loop micro-service provides the following endpoints:

- **POST /games/play/publish/:urlCode:** Publish to game play loop using game URL code.
- **POST /games/play/subscribe/:urlCode:** Subscribe to game play loop using game URL code.
- **POST /games/play/unsubscribe/:urlCode:** Unsubscribe from game play loop using game URL code.

Refer to the [Swagger UI](http://localhost:7011/swagger-ui) documentation for detailed information on the request and response payloads of each endpoint. The Swagger UI provides an interactive interface to explore and test the API endpoints.

## Authentication

The Service-Game-Loop micro-service uses JWT (JSON Web Tokens) for authentication. When a user signs in, an access token is generated and returned in the response. This token should be included in the **`Authorization`** header of subsequent requests as a bearer token.
